...
import { getBrandsReports } from "@/models/reports";
import { BanknoteArrowUp, DollarSign, ShoppingCart, X } from "lucide-react";
import { CompaniesTable } from "@/features/admin/reports/components/companiesTable";
import Link from "next/link";
import { Button } from "@/components/ui/button";

export default async function CompaniesReportsPage(props: {
  ...
  const company = Number(searchParams.company);
  ...
  const data = await getBrandsReports(from, to, company);
  ...
  return (
    <div className="flex flex-col p-6 space-y-8 max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {company > 0 && (
          <div className="flex items-center gap-3 bg-secondary/20 px-4 py-2 rounded-full border border-secondary">
            <span className="text-sm font-medium">عرض نتائج الشركة محدد</span>
            <Link
              href={`/admin/reports/companies?from=${from}&to=${to}`}
      ...
      <CompaniesTable brands={reportData.brands} />
    </div>
  );
}
