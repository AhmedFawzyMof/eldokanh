...
import { sendFCMMessage, sendFCMToTopic } from "@/lib/fcm";
        ...
        for (const admin of adminUsers) {
          if (admin.fid) {
            await sendFCMMessage(
              ...
              `${process.env.NEXTAUTH_URL}/dashboard/orders`
            );
          }
        }

        // Broadcast to every device subscribed to the "new-orders" topic
        // (the admin notification app subscribes here — no DB storage needed).
